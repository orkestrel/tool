...
| `tableSpecOf`          | function | Narrow the database tool's parsed `tables` field back to a strict `TableSpec` — the type-level bridge back from the open `recordShape`-widened parsed value.                                                                              |
| `keysOf`               | function | Narrow the database tool's parsed `keys` field back to a `Readonly<Record<string, string>>` — the `keys`-field twin of `tableSpecOf`.                                                                                                     |
| `rowOf`                | function | Narrow the database tool's parsed `row` / `changes` field back to a plain mutable `Record<string, unknown>`.                                                                                                                              |
...
tableSpecOf({ products: { columns: { name: 'string' } } }) // narrowed back to a strict TableSpec
keysOf({ products: 'id' }) // { products: 'id' }
rowOf({ name: 'Widget' }) // a plain mutable copy
